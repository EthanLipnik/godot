...
The offline solver is deterministic and bounded: compact source triangles, snapped-edge pair merging, a Morton-ordered BVH, generation-stamp blocker deduplication, deterministic blocker-surface admission, interned cell sets, and streaming dependency hashes replace the original editor-scan/eager-texture/superquadratic path. Direct mode substitutes typed texture placeholders only for its isolated command. The `.bvis` format and bake-algorithm fields remain independent contracts, both with the sole unshipped baseline value `1`; there is no migration or compatibility path. The authored blocker-triangle cap affects only which opaque surfaces may certify exclusion. It never removes geometry from the primary or transport candidate universe.
