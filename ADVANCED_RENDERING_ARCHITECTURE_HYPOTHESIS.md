...
**2026-08-23 renderer boundary update:** Flux is a separate desktop
RenderingDevice renderer. It owns its basic raster path, ray-tracing scheduler,
and per-view histories; Forward+ remains the built-in fallback and must not own
Flux ray passes. The current bootstrap is Metal-only evidence, not a Vulkan/RTX
or performance-complete claim.
...
- Target sustained true-stereo 90 Hz on the RTX 5080 through measured full or flux rendering, without counting frame generation.
...
- `Sources/FlowMetalRenderer/Flux/FlowFluxRenderer+MetalFX.swift`
- `Sources/FlowMetalRenderer/Flux/FlowFluxRenderer+Rendering.swift`
- `Sources/FlowMetalRenderer/Flux/FluxRenderer.swift`
- `Sources/FlowMetalRenderer/Flux/FlowFluxRenderer+RayTracing.swift`
...
- **Flux interactive:** rasterized or visibility-buffer primary visibility plus selected ray-traced shadows, reflections, transmission, ambient occlusion, direct/indirect lighting, or limited secondary bounces. Effects are selected by measured cost and perceptual value, not by a fixed promise that every pixel follows a full path.
...
The RTX 5080 streamed target is sustained true-stereo 90 Hz. Full path tracing is the quality reference, not the only permitted real-time execution strategy. Flux interactive rendering is a first-class architecture because a stable 90 Hz result with correct motion, stereo, materials, and secondary effects is more valuable than an intermittently full-path-traced result. The UI and diagnostics must report which effects are rasterized, ray traced, reconstructed, or omitted; “Path Traced” must never silently select a flux path.
...
- dynamic selection of full, flux, and progressive modes by quality preset;
...
#### 7.5.1 Streamed static-cluster refinement

The complete implementation hypothesis for promoting this bounded slice into product-neutral Flux virtual geometry is `misc/path_tracing/VIRTUAL_GEOMETRY_IMPLEMENTATION_ARCHITECTURE.md`. It defines the deep compiler and format, renderer-owned residency, Metal mesh-shader and stable ray-structure paths, portable Vulkan BLAS/TLAS, optional NVIDIA cluster acceleration, city integration, stereo ownership, diagnostics, failure behavior, and measured milestone gates. The document remains subordinate to the required outcomes and parity rules here; it does not convert the provisional slice below into accepted production behavior.

The first product-neutral streamed-geometry slice refines that rule into a shared semantic boundary. Large external glTF buffers remain 64-bit file-backed, and `EXT_meshopt_compression` views are range-read and decoded independently rather than materializing a multi-gigabyte source or fallback buffer. Offline output is a versioned manifest plus independently addressable pages with stable IDs, revisions, conservative bounds, geometric error, material slots, parent/child links, and a persistent coarse group. No page blob or GPU vertex/index allocation may exceed the rendering-device `uint32` limit.

Residency follows deterministic `requested -> loading -> resident -> active -> retiring` states under explicit byte/task budgets. Upload completion is an explicit token before a page can become visible, and storage retirement is an explicit token after the last possible GPU reference. Refinement replaces a coarse group only when every selected child is active, so a partial upload cannot create a hole. The resident set is the conservative union of both eye influence regions; geometry and acceleration structures remain view-independent while per-eye tracing and history remain independent.

Shared code exposes ordinary stable triangle surfaces, not a vendor acceleration-structure primitive. The Metal adapter therefore consumes resident pages through conventional BLAS/TLAS build and reuse behavior. A later Vulkan adapter may use Godot's existing ordinary acceleration-structure primitives; optional cluster acceleration structures can only be an adapter detail. This slice does not establish Vulkan/Windows parity.

Current evidence is deliberately bounded. The ratified meshopt modes and filters, malformed ranges, a sparse source offset above 4 GiB, deterministic two-level cluster pages, persistent fallback, stereo-union selection, atomic activation, explicit retirement, and stable BLAS revisions have automated coverage. A direct per-primitive glTF-to-versioned-streamed-cache compiler now exists: it range-reads and decodes the selected primitive, emits deterministic hierarchy/pages and a coarse fallback without retaining the complete imported scene, and passed a bounded actual-input proof. For the first primitive of the inspected Zorah source (479,502 triangles), that proof emitted 9,380 pages, a 29,690,635-byte cache, a 14,448-byte largest page, a 24,464,544-byte maximum compiled page set, and a 6,066,804-byte maximum decoded view. The source was the 10,001,629,940-byte compressed external bin; the logical 31,690,629,768-byte fallback was not materialized. The run saved 16,990 nodes, skipped 3,162 primitives under the configured one-primitive limit, and explicitly reported 313 unsupported alpha-material semantics. It used two source range reads totaling 4,386,366 bytes (maximum 3,857,103), with 66.99 seconds real time, 598,753,280-byte maximum RSS, and 507,544,896-byte peak footprint. Full-source capacity/import-time/disk proof remains open, as do broader attributes/textures/skinning/material semantics. GLB input remains in-memory and `uint32`-bounded; external buffer reads, page blobs, and rendering-device buffers retain their separate range/size invariants. Ordinary active pages consume the existing `MeshInstance` raster/Metal triangle path; asynchronous production rendering-device uploads, completion/fence integration, and Vulkan/Windows adapter/parity validation remain open. This slice does not claim CLAS or full source/runtime parity.
...
Milestones M0 through M2.5 are intentionally executable without a Windows PC. They may establish a Mac reference implementation, portable product foundations, and a provisional Metal runtime/flux path, but they do not certify the renderer as cross-platform complete. M3 is the first PC-dependent milestone and the only place where Vulkan/RTX parity claims become eligible. Work after a failed gate may continue only when it is independent of the failed claim; dependent features remain provisional and must not be relabeled complete.
...
| M2.5 Mac runtime flux renderer | In progress; executable provisional implementation | Forward+ schedules native Metal BLAS build/in-place refit/TLAS reuse; ray-traced directional visibility; a weighted per-view direct-light reservoir with temporal/one-neighbor spatial reuse; and bounded one-bounce diffuse transport. Its punctual proposal mixes a camera-centered 4 m/32-light ReGIR-style window with a global proposal, while screen histories remain per view. Secondary local transport serializes Omni, untextured Spot, and finite rectangular Area records with linear color/energy/indirect-energy conversion, transform, range/attenuation, cone/area shape, cull mask, finite visibility, and no fixed 16-light upload cap. Secondary triangle albedo uses an approximate world/UV ray-cone mip footprint. Flux MetalFX uses a distinct transparency overlay rather than compositing alpha into the denoised base. A revisioned 64³ diffuse-only cache, split diffuse/specular moments, authored `EnvironmentPortal3D` extraction, portal/environment MIS, and variance-bounded work controls are wired. Negative/projected/textured/degenerate-light cases remain explicitly diagnosed. The Mac-only material/guide/MetalFX work remains provisional. | MSL syntax and source contracts pass, but no fresh runtime/image evidence exists for the Spot/Area, ray-cone, alpha-overlay, portal, cache, reservoir, split-history, or adaptive paths. The Metal emissive distribution remains power times conservative instance extent rather than an exact per-triangle GPU distribution/sample identity; its backend-neutral current/previous mapping ABI is not yet the upload format, and the ReGIR window is shader-built rather than a production scene resource. This remains Mac-only and one-bounce—not full analytic-light/material parity, a performance acceptance, Windows parity, DLSS, true stereo, or the RTX 5080 90 Hz target. M2.5 has not passed. |
...
### M2.5 — Mac runtime and WYSIWYG flux foundation
...
The first runtime slice further falsified post-compositing shadow multiplication. Multiplying visibility over already-lit raster color darkens ambient and indirect energy and double-applies shadowing. The restored interactive split therefore keeps authored direct-light visibility inside Flux raster and adds separately reconstructed diffuse/specular ray transport after opaque shading. A future ray-shadow replacement must be integrated at the individual light's visibility term; it may not multiply final color.

Deliverables:

- integrate a capability-gated flux renderer into the normal Forward+ viewport/game frame graph rather than a separate editor preview;
...
- expose truthful `Raster`, `Flux`, and `Progressive Reference` modes, with diagnostics listing which effects are rasterized, traced, reconstructed, disabled, or unsupported;
...
Gate: on the recorded Mac, the same saved validation scene and camera must produce equivalent named flux effects in the editor viewport and a running game, update motion/deformation without stale acceleration structures, evaluate supported secondary-hit materials, cover the documented light families, reject history at motion/disocclusion, and sustain an explicitly recorded interactive test profile without full scene extraction every frame. This authorizes the label **provisional Mac runtime flux renderer** only. It does not authorize Vulkan parity, Windows performance, streamed XR, or the RTX 5080 90 Hz claim.
...
- implement and expose full, flux, and progressive quality modes with truthful diagnostics;
- implement the shared flux path on Metal and Vulkan, using raster/visibility primary hits and measured ray-traced secondary effects where that is the winning quality/performance allocation;
...
| H11 | A measured full or flux renderer can sustain true-stereo 90 Hz on the RTX 5080 at the frozen production profile. | M4 ten-minute fixture with p99 CPU/GPU frame times, fixed quality floors, temporal captures, and no frame generation. | Reduce effect allocation through an explicitly named flux preset, improve reconstruction, or revise the content profile; never silently lower the frozen gate. |
...
| H13 | ReSTIR GI, ReSTIR PT/GRIS, and a shared world-space radiance cache are worthwhile indirect-transport complements. | Run separate fixed-cost, progressive-reference, correlation/disocclusion, memory, and stage-timing prototypes after DI is correct. | Retain the lower-complexity DI/flux path; do not couple indirect methods to the shared ABI prematurely. |
...
Flux rendering, reconstruction, adaptive resolution, LOD, and temporal reuse can meet a timing graph while visibly breaking motion or materials. Freeze the M4 profile before optimization, compare every accepted change against progressive references and temporal sequences, and expose the active technique in diagnostics. A hidden quality reduction is a correctness failure, not an optimization.

### XR latency and sickness

The target is sustained 90 Hz stereo on the RTX 5080, not a beautiful low-frame-rate demo. Measure the complete pipeline and provide truthful full/flux quality modes, stable pacing, prediction, comfort locomotion, and rapid fallback. Do not use averages alone; track p99/worst-frame behavior and latency distributions. Frame generation does not satisfy the initial target.
...
13. Integrate the provisional Metal flux path into the actual Forward+ editor/game viewport frame graph, replace full scene-tree capture with renderer-owned incremental state, and pass the M2.5 WYSIWYG/runtime gate.
...
16. Execute M4 as a concentrated cross-backend performance program: freeze the production profile, optimize the already-correct flux path and the remaining families in section 7.2, and meet sustained true-stereo 90 Hz on the RTX 5080.
...
### 2026-08-22 baked-visibility primary/transport contract

The existing `.occ` workflow bakes triangle occluders for per-frame camera visibility; it does not preserve the dependency closure required by flux reflection, GI, emissive, or finite-light transport. Reusing that primary list for ray transport was therefore the original correctness failure: a primary-invisible object can still alter a visible receiver.

`BakedVisibilityData3D` stores a versioned, CRC-protected `.bvis` payload with source UID/path/digest, anchor-local grid and tolerances, lexically sorted relative `NodePath` keys, stable local bounds and SHA-256 signatures, per-cell primary/transport set IDs, and deterministic tiled cell-index coverage. It never serializes session RIDs or ObjectIDs. At runtime, the anchor resolves paths relative to its bake root and validates scenario, exact layer mask, source digest, transform/bounds/version/signature/material state, and every eye's margin-covered cells. Any uncertainty—including corrupt data, stale source, unresolved path, changed/hidden certified blocker, cell degradation, boundary escape, or incompatible settings—fails open.

The primary set may reduce camera-color geometry admission only. It never culls shadows, probes, SDFGI, particles, or notifiers. The transport set remains a conservative flux geometry/light closure: for matching `D`, static registered transport members are used together with all dynamic/unbound candidates; a dynamic/unbound primary or `D` mismatch retains the current full runtime closure. This preserves off-camera reflection/GI/light influence while allowing safe static-map savings. Stereo coverage is a union across eyes and volumes, with per-eye histories still independent.

The deterministic fixture is `misc/path_tracing/m2_5/validation_project/baked_visibility_fixture.tscn`. Create its resource first with the direct headless command. It dispatches before `SceneTree`/`EditorNode` and requires imported dependencies to be current:
...
The first command atomically writes the `.bvis` and updates the scene. The second performs anchor enabled/disabled image A/B, verifies a primary-hidden contributor/light affects the visible receiver ROI, and changes an opaque certified blocker to transparent to require fail-open. The renderer diagnostic reports primary/transport retained-versus-denominator counts, and the anchor's read-only runtime-stat dictionary exposes the same active query result to the harness; it rejects the run unless primary static geometry is reduced by at least 25% and flux geometry by at least 10%.

Two current-baseline direct fixture bakes produced the same 12,499-byte resource with SHA-256 `f04d172b00ac0fd7c43b7662e23651551c384f3568cef7a33ee2f222c4dde0c2`. The earlier completed runtime fixture reports active primary geometry `5/12`, flux transport geometry `7/13`, and transport lights `2/2`; enabled/disabled MAE `0.000441`, hidden-transport receiver-ROI MAE `0.000452`, and transparent-certified-blocker fail-open MAE `0.002664`. These pass its reduction, retained-transport, and fail-open gates. A current-tree rerun was stopped at 547,616 KiB by the unchanged 512 MiB machine guard during Metal environment allocation, before image assertions; current-renderer runtime acceptance therefore remains open. This is bounded-correctness evidence, not a multibounce, stereo, Vulkan/RTX, or 90 Hz claim.

The offline solver is deterministic with explicit stage bounds: compact source triangles, snapped-edge pair merging, a Morton-ordered BVH, generation-stamp blocker deduplication, deterministic blocker-surface admission, interned cell sets, and streaming dependency hashes replace the original editor-scan/eager-texture/superquadratic path. Direct mode substitutes typed texture placeholders only for its isolated command. The `.bvis` format and bake-algorithm fields remain independent contracts, both currently `3`, and reject every prior payload without migration or compatibility paths. The authored blocker-triangle cap affects only which opaque surfaces may certify exclusion. It never removes geometry from the primary or transport candidate universe.

Danger-room coverage is currently `8/11`, not complete. Eight strict, non-degraded baseline resources were regenerated twice under a serialized 512 MiB/120 s guard and matched byte-for-byte, with a maximum second-pass RSS of about 246 MiB. Bistro, Emerald, and Zero-Day have no output or partial file because their imported PackedScenes cross the guard before visibility preprocessing. Isolated loads reproduce the boundary for Bistro exterior (`618624 KiB`), Bistro hallucinated (`530096 KiB`), and Emerald (`560112 KiB`); Zero-Day's cached PackedScene alone is `227 MiB` compressed. Finishing those scenes requires a bounded imported-geometry sidecar/proxy or identity-preserving asset partitioning. Emitting an all-visible resource would be safe but useless and is not accepted as a completed bake.
...
The reported razor-straight editor line was separately falsified as renderer output: it is the editor origin/grid gizmo layer, which is deliberately in the editor camera's cull mask and continues outside scene content. A clean game capture does not contain it. Do not use global reactive/denoise masking to accommodate editor UI. The provisional Metal Flux path now allocates a distinct linear RGBA transparency target, renders camera-visible alpha into it after opaque flux transport, and supplies it exactly once to MetalFX's transparency-overlay input. The opaque base therefore remains the only denoised color signal. Ordinary raster/non-denoised paths retain their existing alpha composition, and editor gizmos remain an editor-owned full-resolution overlay. This is a Metal frame-graph implementation detail, not Vulkan/RTX parity or a completed alpha-material transport contract; failure recovery and runtime alpha-fixture evidence remain gates.
...
The following is baseline evidence from the former visible-Sky-only procedural atmosphere, superseded for Metal Flux by the residual/lobe experiment recorded in Section 7.9. Its finite 3.2-degree authored solar disk was separate from the diffuse dome and attenuated by the same bounded cloud field. A Mac-only fixed-capture run found finite, selectable RGBA32Float Sky importance data at twilight and night, but did not establish a measurable moving hard-shadow centroid from that full-Sky disk. This baseline falsifies neither the implemented residual-lobe path nor the unchanged-Sky MIS hypothesis; it retains the no-double-count comparison as a required follow-up. Do not infer a direct-light, cloud-shadow, backend-parity, stereo, or performance result from these baseline captures.
...
### 2026-08-22 transport-conservative flux scene selection hypothesis
...
This work is not a `.occ` format extension, baked PVS system, or a claim of multibounce correctness, stereo certification, Vulkan parity, or performance. It preserves primary raster behavior while making the flux ray scene conservative for the present one-secondary-segment transport model. Useful primary-visibility context is [Teller & Séquin's PVS work](https://people.csail.mit.edu/teller/pubs/siggraph91.pdf) and [GPU Gems 2's visibility management for per-pixel lighting](https://developer.nvidia.com/gpugems/gpugems/part-ii-lighting-and-shadows/chapter-15-managing-visibility-pixel-lighting); the transport distinction follows the path-space framing in [Veach's thesis](https://graphics.stanford.edu/papers/veach_thesis/).
## 2026-08-23 — ray-geometry LOD exactness policy

Generated mesh LOD is a performance tool for the ray world, not a valid approximation for every instance. The renderer may select generated ray LOD only for far, static, opaque geometry. Deforming/skinned/morph geometry, strict alpha-masked surfaces, invalid camera/bounds inputs, and geometry whose world AABB is within `rendering/flux/ray_tracing/ray_geometry_lod/near_field_distance` retain base triangles. The default threshold is 10 m. Distance is measured to the closest point of the world AABB so large nearby objects do not qualify merely because their origin or center is distant.

This policy preserves progressive BLAS budgeting and far-world triangle reduction. Diagnostics separate selected LOD surfaces from base retention caused by dynamic, alpha-mask, and near-field rules. The pure policy is backend-neutral; the current runtime evidence is Metal-only and does not establish Vulkan/RTX parity, stereo performance, or the 90 Hz production gate.

## 2026-08-23 — bounded ray-world convergence order

A hard per-frame BLAS budget is necessary but insufficient: arbitrary stable-ID admission can make a dark emissive world formally progress while remaining visually useless. Missing ray geometry should converge in a deterministic quality order: dynamic geometry first, emissive geometry second, then nearest static geometry, with stable identity only as the final tie-break. Existing BLAS reuse/refit is not charged to the new-build budget. The priority is a scheduling rule, not visibility culling; deferred geometry remains eligible and is retried.

Zero-Day falsified stable-ID-only ordering: an eight-frame bounded run residented one of fourteen emissive materials and remained black. With priority ordering, the same 32-build cap residented all fourteen and produced the expected colored emissive corridor with zero analytic lights and zero texture fallback. This fixes early convergence, not steady-state capacity. The measured stress scenes still require conservative coarse ray proxies, transport-aware cell/cluster streaming, and a cheaper TLAS/secondary-ray representation before interactive performance is plausible.

## 2026-08-23 — binary Standard/Flux renderer contract

The public renderer selection is binary: `Standard` uses built-in Godot Forward+; `Flux` enables the complete custom flux feature set. There is no public partial, realtime, or shadow-only renderer mode. Project setting and explicit viewport value `1` mean Flux; value `0` means Standard; a viewport may use `-1` only to inherit the project setting. The editor's opt-in Flux Preview restores inheritance when enabled and applies an explicit Standard override when disabled.

The former partial-mode measurements remain useful only as historical performance evidence for shader classification and bounded geometry admission. They are not a supported renderer contract, quality setting, or fallback. Flux's current performance, stereo, Vulkan/RTX, and 90 Hz gates remain open.

## 2026-08-24 — baked visibility format cutover

Baked visibility now uses a hard-cut current contract; obsolete payloads are rejected rather than migrated or dual-decoded. The CPU reference baker constructs an immutable scene snapshot, deterministic Morton BVHs, parallel worker-local leaf-tile certificates, and a canonical parent/child tile hierarchy. Metadata has its own CRC; each leaf owns a bounded length-delimited cell/set chunk with its own CRC, so loading retains only metadata and immutable leaf bytes. Runtime decodes and caches no more than eight requested leaves, walking parent metadata to route lookup; a corrupt leaf fails open only for queries that require it. Incomplete work is persisted outside the runtime resource as a checksummed, atomically replaced checkpoint; it contains immutable settings, canonical dependency paths, complete-tile bitmap, and the exact union of candidate/light membership plus every blocker actually used by per-cell exclusion certificates. Resume recomputes that union before admission and only reuses an identical leaf; stale settings and changed dependencies reject reuse. The baker reserves its bounded extraction/BVH/worker/result/checkpoint ownership estimate before per-cell candidate allocation; this is not a cap on unrelated renderer or process memory. Unsupported or over-cap certificates remain fail-open.

The optional product-neutral batch contract carries immutable candidate and blocker bounds, a discovery mask, canonical compaction order, and blocker-hit hints. Its CPU oracle is always compared before accelerator output is admitted, and no hint can suppress the later conservative CPU certificate. Native Metal builds blocker BLAS/TLAS objects and dispatches candidate classification, hardware blocker-ray queries, and canonical compaction/sort through a callback with explicit buffer-resource lifetime declarations. The focused macOS fixture requires actual execution and passed on 2026-08-24: `bin/godot.macos.editor.dev.arm64.bvistests --test --test-case='*native batch matches CPU candidate semantics*' --no-window-focus --audio-driver Dummy --exit` reported 1/1 case and 10/10 assertions, including three GPU dispatches, one hardware-ray query per candidate, and CPU-equivalent mask/order/hints. The complete focused suite reported 49/49 cases and 2,608/2,608 assertions. The Metal-enabled editor/test binary was built with `metal=yes`, `tests=yes`, `vulkan=no` in 00:03:27.55. Generic Vulkan source builds BLAS/TLAS, ray-generation/miss/closest-hit stages, descriptor bindings, SBT, per-candidate `traceRayEXT` dispatch, readback, and CPU-oracle comparison behind `SUPPORTS_RAYTRACING_PIPELINE` and `SUPPORTS_RAY_QUERY`; it falls back to CPU when unavailable. Windows Vulkan/RTX runtime execution and parity are still an explicit external gate: the compileable source/feature-fallback contract is covered locally, but no Windows execution is claimed.
