...
The existing `.occ` workflow bakes triangle occluders for per-frame camera visibility; it does not preserve the dependency closure required by hybrid reflection, GI, emissive, or finite-light transport. Reusing that primary list for ray transport was therefore the original correctness failure: a primary-invisible object can still alter a visible receiver.
...
The primary set may reduce camera-color geometry admission only. It never culls shadows, probes, SDFGI, particles, or notifiers. The transport set remains a conservative hybrid geometry/light closure: for matching `D`, static registered transport members are used together with all dynamic/unbound candidates; a dynamic/unbound primary or `D` mismatch retains the current full runtime closure. This preserves off-camera reflection/GI/light influence while allowing safe static-map savings. Stereo coverage is a union across eyes and volumes, with per-eye histories still independent.
...
The reported razor-straight editor line was separately falsified as renderer output: it is the editor origin/grid gizmo layer, which is deliberately in the editor camera's cull mask and continues outside scene content. A clean game capture does not contain it. Do not use global reactive/denoise masking to accommodate editor UI. The provisional Metal Hybrid path now allocates a distinct linear RGBA transparency target, renders camera-visible alpha into it after opaque hybrid transport, and supplies it exactly once to MetalFX's transparency-overlay input. The opaque base therefore remains the only denoised color signal. Ordinary raster/non-denoised paths retain their existing alpha composition, and editor gizmos remain an editor-owned full-resolution overlay. This is a Metal frame-graph implementation detail, not Vulkan/RTX parity or a completed alpha-material transport contract; failure recovery and runtime alpha-fixture evidence remain gates.
...
The editor Preview Sun is a real shadowed `DirectionalLight3D`, so leaving it active while Hybrid explicitly owns a scene's `WorldEnvironment` double-counts direct lighting and makes the editor viewport disagree with sky-only runtime output. The 3D editor now suppresses that internal light only when all three ownership conditions hold: a `WorldEnvironment` exists, Hybrid mode `1` is selected, and hybrid environment lighting is enabled. The Preview Sun button is disabled and its state text identifies Hybrid environment ownership. Outside this predicate, authored directional-light suppression and the ordinary manual Preview Sun toggle retain their existing behavior. This is editor-only ownership hygiene; it does not synthesize a light or alter environment transport, energy, exposure, game runtime, or backend parity.
...
The following is baseline evidence from the former visible-Sky-only procedural atmosphere, superseded for Metal Hybrid by the residual/lobe experiment recorded in Section 7.9. Its finite 3.2-degree authored solar disk was separate from the diffuse dome and attenuated by the same bounded cloud field. A Mac-only fixed-capture run found finite, selectable RGBA32Float Sky importance data at twilight and night, but did not establish a measurable moving hard-shadow centroid from that full-Sky disk. This baseline falsifies neither the implemented residual-lobe path nor the unchanged-Sky MIS hypothesis; it retains the no-double-count comparison as a required follow-up. Do not infer a direct-light, cloud-shadow, backend-parity, stereo, or performance result from these baseline captures.
...
Generated mesh LOD is a performance tool for the ray world, not a valid approximation for every instance. The renderer may select generated ray LOD only for far, static, opaque geometry. Deforming/skinned/morph geometry, strict alpha-masked surfaces, invalid camera/bounds inputs, and geometry whose world AABB is within `rendering/hybrid_renderer/ray_geometry_lod/near_field_distance` retain base triangles. The default threshold is 10 m. Distance is measured to the closest point of the world AABB so large nearby objects do not qualify merely because their origin or center is distant.
...
## 2026-08-23 — binary Standard/Hybrid renderer contract

The public renderer selection is binary: `Standard` uses built-in Godot Forward+; `Hybrid` enables the complete custom hybrid feature set. There is no public partial, realtime, or shadow-only renderer mode. Project setting and explicit viewport value `1` mean Hybrid; value `0` means Standard; a viewport may use `-1` only to inherit the project setting. The editor's opt-in Hybrid Preview restores inheritance when enabled and applies an explicit Standard override when disabled.

The former partial-mode measurements remain useful only as historical performance evidence for shader classification and bounded geometry admission. They are not a supported renderer contract, quality setting, or fallback. Hybrid's current performance, stereo, Vulkan/RTX, and 90 Hz gates remain open.
